...
class Struct extends Component {
  ...
  onChange(fieldName, fieldValue, path, kind=null) {
...
export class List extends Component {
  ...
  removeItem(i) {
    if (this.state.value[i] && this.props.options.removeItemCallback) {
      this.props.options.removeItemCallback()
        .then(() => this._removeItem(i), noop)
    } else {
      this._removeItem(i)
    }
  }
